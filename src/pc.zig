...
pub fn findPc(allocator: Allocator, game: GameState, min_height: u8, comptime max_pieces: usize) ![]Placement {
    return PC(max_pieces).findPc(allocator, game, min_height);
}
...
pub fn PC(comptime max_pieces: usize) type {
    return struct {
        ...
        /// Returns an error if no perfect clear exists, or if the number of pieces needed
        /// exceeds `max_pieces`.
        pub fn findPc(allocator: Allocator, game: GameState, min_height: u8) ![]Placement {
            ...
            if (!isPcPossible(game.playfield.rows[0..field_height])) {
                return FindPcError.NoPcExists;
            }

            // If the playfield is completely empty, at least 5 pieces are needed to achieve
            // a perfect clear. If the number of empty cells is not a multiple of 4, 1 extra
            // line (aka 10 cells) is needed.
            var pieces_needed = if (bits_set == 0)
                5
            else if (empty_cells % 4 == 2)
                (empty_cells + 10) / 4
            else
                empty_cells / 4;
        ...
        fn findPcInner(
            ...
            @memcpy(&node.rows, game.playfield.rows[0..MAX_HEIGHT]);
